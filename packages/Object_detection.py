# -*- coding: utf-8 -*-
from .object_config import MIN_CONFID 
from .object_config import NMS_THRESHLD
...
def getOutput(output, boxes, centroids, confidences, H, W, personIdx = 0):
  ret_list = Parallel(n_jobs = multiprocessing.cpu_count())(delayed(getDetection)(detection, boxes, centroids, confidences, H, W, personIdx) for detection in output)

def getDetection(detection, boxes, centroids, confidences, H, W, personIdx = 0):
  scores = detection[5:]
...
def detect_people(frame, net, ln, personIdx=0):
    ...
    # ret_list = Parallel(n_jobs = multiprocessing.cpu_count())(delayed(getOutput)(output, boxes, centroids, confidences, H, W, personIdx) for output in layerOutputs)
